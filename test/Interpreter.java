...
class Interpreter implements Expr.Visitor<Object>, Stmt.Visitor<Void> {
  ...
  public Object visitBinaryExpr(Expr.Binary expr) {
    ...
    switch (expr.operator.type) {
      ...
      case SLASH: {
        ...
        if (denom == 0.0) throw new RuntimeError(expr.operator, "Division by zero.");
  ...
  private double asFlow(Token op, Object v) {
    if (v instanceof Double d) {
      return d; // allow plain numbers to behave as flows if used with flow ops
    }
    if (isFlowVal(v)) {
      String s = (String) v;
      String core = s.substring(0, s.length() - 1); // drop trailing 'x'
      try {
        return Double.parseDouble(core);
      } catch (NumberFormatException e) {
        throw new RuntimeError(op, "Invalid flow literal: " + s);
      }
    }
    throw new RuntimeError(op, "Expected a flow value (e.g., 3.2x) or number, got: " + stringify(v));
  }
